...
export const getAllCustomersOrder = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.get(`/api/v1/order?page=${orderpage}`, config);
...
export const updateCustomersOrderToPaid = createAsyncThunk(
  ...
  async (details, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.put(`/api/v1/order/${_id}/pay`, details, config);
...
export const updateCustomersOrderToIsDelivered = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { _id } = state.order.order
      const { data } = await axios.put(`/api/v1/order/${_id}/delivered`, name, config);
