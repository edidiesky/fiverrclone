...
export const GetAllBuyerCart = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      let cartUrl = `${import.meta.env.VITE_API_BASE_URLS}/api/v1/cart/${id}`;
...
export const GetSingleBuyerCart = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    ...
    try {
      let cartUrl = `${import.meta.env.VITE_API_BASE_URLS}/api/v1/cart/buyer/${name}`;
  ...
  async ({ qty }, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/cart/${state.gigs.GigsDetails._id}`,
        { qty },
        config
      );
  ...
  async (GigsData, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/gig/admin/${_id}`,
        GigsData,
        config
      );
  ...
  async (cartid, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/gig/admin/${cartid}`,
        config
      );
      return cartid;
    } catch (error) {
