...
export const getAllReviews = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    try {
      let reviewUrl = `${import.meta.env.VITE_API_BASE_URLS}/api/v1/review`;
...
export const getSellerReviews = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/${name}`);
  ...
  async (reviewsData, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/admin/${_id}`,
        reviewsData,
        config
      );
  ...
  async (reviewsid, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/admin/${reviewsid}`,
        config
      );
      return reviewsid;
    } catch (error) {
  ...
  async ({ Reviewdata, id }, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/${id}`,
        Reviewdata,
        config
      );
      return data.review;
