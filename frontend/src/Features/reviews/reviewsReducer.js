...
export const UpdateReviews = createAsyncThunk(
  ...
  async (reviewsData, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.put(`/api/v1/review/admin/${_id}`, reviewsData, config);
...
export const DeleteReviews = createAsyncThunk(
  ...
  async (reviewsid, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.delete(`/api/v1/review/admin/${reviewsid}`, config);
      return reviewsid;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response && error.response.data.message
        ? error.response.data.message : error.message);
...
export const createReviews = createAsyncThunk(
  ...
  async ({ Reviewdata, id }, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.post(`/api/v1/review/${id}`, Reviewdata, config);
      return data.review;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response && error.response.data.message
        ? error.response.data.message : error.message);
